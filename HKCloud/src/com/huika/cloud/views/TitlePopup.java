...
public class TitlePopup extends PopupWindow {
	...
	public TitlePopup(Context context, int width, int height,
			OnItemOnClickListener listener, int ImagePos) {
		this(context, R.layout.title_popup, width, height, listener, ImagePos);

	}

	public TitlePopup(Context context, int layoutResID, int width, int height,
			OnItemOnClickListener listener, int ImagePos) {
		super(context);
	...
	private void initUI() {
		...
		mTitleLinearLayout = (LinearLayout) getContentView().findViewById(
				R.id.title_popu_linear);
...
//		}
		populateActions();
		// if (LayoutParams.MATCH_PARENT == getHeight()) {
		// setHeight(Math.abs(getHeight()) - mRect.bottom);
		// }
		// 显示弹窗的位置
...
//						textView.setGravity(Gravity.CENTER);
		...
		mLayoutParams.width = mScreenWidth / 1;
