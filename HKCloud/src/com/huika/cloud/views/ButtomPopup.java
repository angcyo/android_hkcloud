...
public class ButtomPopup extends PopupWindow {
	...
	private HKCloudApplication appBaseApplication;
	// 定义弹窗子类项列表
	private ArrayList<ActionPopupItem> mActionItems = new ArrayList<ActionPopupItem>();
	private OnClickListener dismissClickListener;

	public ButtomPopup(Context mContext, int width, int height, OnPopupItemOnClickListener listener) {
		super(mContext);
		this.mContext = mContext;
		this.mItemOnClickListener = listener;
		setContentView(LayoutInflater.from(mContext).inflate(R.layout.popup_actionsheet, null));
		// 设置弹窗的宽度和高度
		setWidth(width);
		setHeight(height);
		setFocusable(true);
		...
		update();
		appBaseApplication = HKCloudApplication.getInstance();
		// 获得屏幕的宽度和高度
		// 获得屏幕的宽度和高度
	...
	private void controlPopSelect(int index) {
		for (ActionPopupItem item : mActionItems) {
		...
		mListView.setAdapter(new BaseAdapter() {
			...
			public View getView(int position, View convertView, ViewGroup parent) {
				...
				if (convertView == null) {
					...
					convertView = View.inflate(mContext, R.layout.pop_item_action, null);
