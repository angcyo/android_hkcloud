...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
	...
	public final boolean onInterceptTouchEvent(MotionEvent event) {
		...
		switch (action) {
			case MotionEvent.ACTION_MOVE: {
				...
				if (!mScrollingWhileRefreshingEnabled && isRefreshing()) { return true; }
	...
	public static enum AnimationStyle {
		...
		/**
		 * This is the self define type. Uses only an
		 * arrow image flips depending on where the user has scrolled.
		 */
		OTHER;
	...
	public static enum Mode {
		...
		static Mode mapIntToValue(final int modeInt) {
			for (Mode value : Mode.values()) {
				if (modeInt == value.getIntValue()) { return value; }
			}
	...
	public static enum State {
		...
		static State mapIntToValue(final int stateInt) {
			for (State value : State.values()) {
				if (stateInt == value.getIntValue()) { return value; }
			}
