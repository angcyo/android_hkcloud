...
public class GridViewWithHeaderAndFooter extends GridView {
	...
	public void addHeaderView(View v, Object data, boolean isSelectable) {
		ListAdapter adapter = getAdapter();
		if (adapter != null && !(adapter instanceof HeaderViewGridAdapter)) { throw new IllegalStateException("Cannot add header view to grid -- setAdapter has already been called."); }
	...
	public void addFooterView(View v, Object data, boolean isSelectable) {
		...
		if (mAdapter != null && !(mAdapter instanceof HeaderViewGridAdapter)) { throw new IllegalStateException("Cannot add header view to grid -- setAdapter has already been called."); }
	...
	private int getNumColumnsCompatible() {
		if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB) {
			...
			try {
				...
				if (mNumColumns != -1) { return mNumColumns; }
	...
	public int getRowHeight() {
		if (mRowHeight > 0) { return mRowHeight; }
		ListAdapter adapter = getAdapter();
		int numColumns = getNumColumnsCompatible();

		// adapter has not been set or has no views in it;
		if (adapter == null || adapter.getCount() <= numColumns * (mHeaderViewInfos.size() + mFooterViewInfos.size())) { return -1; }
	...
	private static class HeaderViewGridAdapter implements WrapperListAdapter, Filterable {
		...
		public boolean isEnabled(int position) {
			...
			if (position < numHeadersAndPlaceholders) { return position % mNumColumns == 0 && mHeaderViewInfos.get(position / mNumColumns).isSelectable; }
			...
			if (mAdapter != null) {
				...
				if (adjPosition < adapterCount) { return adjPosition < mAdapter.getCount() && mAdapter.isEnabled(adjPosition); }
			}
		...
		public Object getItem(int position) {
			...
			if (position < numHeadersAndPlaceholders) {
				if (position % mNumColumns == 0) { return mHeaderViewInfos.get(position / mNumColumns).data; }
				return null;
			}
